 ...
 * =========================LICENSE_END==================================
 */
package it.csi.pslp.pslpbff.api.impl.generic;
...
import it.csi.pslp.pslpbff.api.dto.TipoMessaggio;
import it.csi.pslp.pslpbff.api.dto.Utente;
import it.csi.pslp.pslpbff.api.dto.common.CommonResponse;
import it.csi.pslp.pslpbff.api.dto.mappers.PslpMappers;
...
public class BaseApiServiceImpl {
    ...
    protected <R> Response buildManagedResponseKOEsterna(HttpHeaders httpHeaders, R responseContent) {
        ...
        try (CloseableHttpResponse response = httpClient.execute(request)) {
            try {
                ...
                if (statusCode == 200) {
                    ...
                    throw BusinessException.createError("Errore chiamata servizio status : " + statusCode);
                }
            } catch (Exception e) {
                Log.error(e);
                throw BusinessException.createError(e.getMessage());
            } finally {
    ...
    // genera cal info per SILP
    protected CallInfo creaCallInfo() {
        ...
        callInfo.setCodFiscale(utente.getCfUtente());
        callInfo.setCognomeOperatore(utente.getCognome());
        callInfo.setNomeOperatore(utente.getNome());
        // callInfo.setIdRuolo(utente.getRuoli().get(0).getIdRuolo());
        return callInfo;
    }

    // genera cal info per BLP
    protected it.csi.pslp.pslpbff.api.dto.blp.CallInfo creaCallInfoBlp() {
        Utente                                   utente   = PslpThreadLocalContainer.UTENTE_CONNESSO.get();
        it.csi.pslp.pslpbff.api.dto.blp.CallInfo callInfo = new it.csi.pslp.pslpbff.api.dto.blp.CallInfo();
        callInfo.setIdAppChiamante(PslpConstants.COMPONENT_NAME_CHIAMANTE);
        callInfo.setCodFiscale(utente.getCfUtente());
        // callInfo.setIdRuolo(utente.getRuoli().get(0).getIdRuolo());
        return callInfo;
