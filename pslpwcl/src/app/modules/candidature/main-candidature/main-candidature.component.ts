...
@Component({
  selector: 'pslpwcl-main-candidature',
  templateUrl: './main-candidature.component.html',
  styleUrls: ['./main-candidature.component.scss']
})
export class MainCandidatureComponent implements OnInit {
          ...
          next: async (res: any) => {
            ...
            if (!chkPrivacy) {
              const data: DialogModaleMessage = {
                titolo: "Gestione candidature",
                tipo: TypeDialogMessage.YesOrNo,
                messaggio: "PRIVACY DA CONFERMARE PER candidature</br></br> Scegliendo SI, " +
          ...
          next: async res => {
            ...
            if (res.esitoPositivo && res.fascicolo) {
              ...
              const data: DialogModaleMessage = {
                ...
                messaggio: 'Non è presente un fascicolo. Non è possibile visualizzare le candidature',
                size: "lg",
                tipoTesto: TYPE_ALERT.WARNING
              };
              const result = await this.promptModalService.openModaleConfirm(data);
  ...
  ngOnInit(): void {
     this.messagioService.findByCod("I58").subscribe({
      next: (res: any) => {
