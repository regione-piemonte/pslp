# Prodotto
PSLP componente PSLPAPI (esposizione API di integrazione con altri servizi)


# Descrizione della componente
Questa cartella rappresenta una "unità di installazione" corrispondente alla componente "logica" di backend che espone servizi verso altri applicativi dei sistemi informativi regionali del lavoro e della formazione professionale; in particolare espone servizi utili all'integrazione con il portale IoLavoro per alimentare la Banca Dati BLP Borsa Lavoro Piemonte.

In generale, è predisposta per l'esposizione di API REST e si connette al DB (PSLPDB) per le operazioni CRUD correlate ai servizi esposti.
Gli endpoint sono esposti tramite protocollo HTTP, e protetti tramite uno schema di "basic authentication".
